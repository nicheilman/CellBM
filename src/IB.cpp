...
for(int i=0; i<mesh_size_[0]+1; i++){
		...
		IB_node_lst_.push_back(std::make_shared<IB_node>( i*(1.2/mesh_size[0])+com_[0]-0.1, rad_*cos(j*(2*PI/mesh_size[1]) )+com_[1], -1*rad_*sin(j*(2*PI/mesh_size[1]) )+com_[2], i*mesh_size_[1]+j));
...
bool IB::in_bbox(std::shared_ptr<node> node, double mesh_space){
